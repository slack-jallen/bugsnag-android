...
Then("the exception reflects a signal was raised") do
  ...
  Test::Unit::Assertions.assert_block("The errorClass was not from a signal: #{error_class}") do
...
Then("the exception {string} equals one of:") do |keypath, possible_values|
  ...
  Test::Unit::Assertions.assert_includes(possible_values.raw.flatten, value)
end
...
Then("the error payload contains a completed unhandled native report") do
  ...
  stack = Maze::Helper.read_key_path(Maze::Server.errors.current[:body], "events.0.exceptions.0.stacktrace")
    stack.each_with_index do |frame, index|
      Test::Unit::Assertions.assert_not_nil(frame['symbolAddress'], "The symbolAddress of frame #{index} is nil")
      Test::Unit::Assertions.assert_not_nil(frame['frameAddress'], "The frameAddress of frame #{index} is nil")
      Test::Unit::Assertions.assert_not_nil(frame['loadAddress'], "The loadAddress of frame #{index} is nil")
    end
end
...
Then("the stacktrace contains native frame information") do
  ...
  stack = Maze::Helper.read_key_path(Maze::Server.errors.current[:body], "events.0.exceptions.0.stacktrace")
  stack.each_with_index do |frame, index|
    Test::Unit::Assertions.assert_not_nil(frame['method'], "The method of frame #{index} is nil")
    Test::Unit::Assertions.assert_not_nil(frame['lineNumber'], "The lineNumber of frame #{index} is nil")
  end
end
...
Then("the exception stacktrace matches the thread stacktrace") do
  ...
  Test::Unit::Assertions.assert_equal(exc_trace.length(), thread_trace.length(), "Exception and thread stacktraces are different lengths.")

  thread_trace.each_with_index do |thread_frame, index|
    exc_frame = exc_trace[index]
    Test::Unit::Assertions.assert_equal(exc_frame, thread_frame)
  end
end
...
Then("the event binary arch field is valid") do
  ...
  Test::Unit::Assertions.assert_block "'#{arch}' is not a valid value for app.binaryArch" do
...
Then("the event stacktrace identifies the program counter") do
  ...
  trace.each_with_index do |frame, index|
    if index == 0
      Test::Unit::Assertions.assert_equal(frame["isPC"], true, "The first frame should be the program counter")
    else
      Test::Unit::Assertions.assert_equal(frame["isPC"], nil, "The #{index} frame should not be the program counter")
    end
  end
end
